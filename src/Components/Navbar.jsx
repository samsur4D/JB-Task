...
const  Navbar = () => {
  ...
  return (
    <div className="contain flex justify-between items-center py-2 lg:py-3 px-3 lg:px-0">
      <Link to={"/"} data-tooltip-id="my-tooltip" data-tooltip-content="BrandQuest" className="text-[2rem] md:text-4xl font-semibold flex-1 lg:flex-none">Brand<span className="text-cyan-700">Quest</span></Link>
      ...
      <ul className="gap-5 items-center text-xl flex"> 
        ...
        {/* {user && <NavLink to={"/additem"} className={({isActive}) => isActive ? "font-bold cursor-pointer text-cyan-700" : "cursor-pointer"}>Add Item</NavLink>} */}
        {user && <NavLink to={"/productpage"} className={({isActive}) => isActive ? "font-bold cursor-pointer text-cyan-700" : "cursor-pointer"}>Home</NavLink>}
      </ul>
